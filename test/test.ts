...
import { SourceStream, WebSourceStream } from './util.js';
...
describe('read from a streamed data chunk', async () => {
  ...
  it('WebStreamReader', async () => {

    const webSourceStream = new WebSourceStream('\x05peter');
    const webStreamReader = new WebStreamReader(webSourceStream);
    ...
    assert.strictEqual(buf.toString('latin1'), 'peter');
